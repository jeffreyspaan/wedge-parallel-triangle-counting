...
__global__ void tc_GPU_kernel(const UINT_t *g_Ap, const UINT_t *g_Ai, const ULONG_t *g_wedgeSum, const ULONG_t start, const ULONG_t stop, const UINT_t num_vertices, ULONG_t *g_total_count) {
	const ULONG_t i = (ULONG_t) blockIdx.x * blockDim.x + threadIdx.x + start;
	UINT_t count = 0;
	...
	if (i < stop) {
#if BINSEARCH_CONSTANT
		v = binary_search_closest_ULONG_constant_GPU(g_wedgeSum, 0, num_vertices, i);
#else
		v = binary_search_closest_ULONG_GPU(g_wedgeSum, 0, num_vertices, i);
#endif
		...
		// double x1 = double (wedges-i_v);
		// double x2 = (double (wedges-i_v) - 0.875);
		// double x3 = (double (wedges-i_v) - 0.875) * 2;
		// double x4 = (sqrt((double (wedges-i_v) - 0.875) * 2) - 0.5);
		// double x5 = (UINT_t) (sqrt((double (wedges-i_v) - 0.875) * 2) - 0.5);

		w_i = d_v - 2 - (UINT_t) (sqrt((double (wedges-i_v) - 0.875) * 2) - 0.5);
		u_i = i_v + w_i + 1 + ( (((ULONG_t) (d_v-w_i) * (ULONG_t) ((d_v-w_i)-1)) >> 1) - wedges );

		// if (d_v == 67086 && i_v == 0) {
		// 	printf("v=%u i=%lu g_wedgeSum[v]=%lu i_v=%lu wedges=%lu x1=%lf x2=%lf x3=%lf x4=%lf x5=%lf u_i=%u\n", v, i, g_wedgeSum[v], i_v, wedges, x1, x2, x3, x4, x5, u_i);
		// }

		// if (w_i < 0 || w_i > d_v) {
		// 	printf("v=%u d_v=%u wedges=%lu w_i=%u\n", v, d_v, wedges, w_i);
		// }

		// if (u_i < 0 || u_i > d_v) {
		// 	printf("v=%u d_v=%u wedges=%lu u_i=%u\n", v, d_v, wedges, u_i);
		// }
...
ULONG_t tc_GPU(const GRAPH_TYPE *graph, GPU_time *t) {
	...
	ULONG_t max_wedges = max_blocks * num_threads; // max wedges in a single launch
	...
	for (ULONG_t start=0; start<wedgeSum_total; start+=max_wedges) {
		...
		tc_GPU_kernel<<<grid, threads>>>(d_Ap, d_Ai, d_wedgeSum, start, stop, graph->numVertices, d_total_count);
...
void usage() {
	...
	printf("\n");
	printf("Example:\n");
	printf("./tc_base -m Amazon0302.mtx -l 10\n");
	exit(EXIT_FAILURE);
}
...
GRAPH_TYPE *preprocess_base(const GRAPH_TYPE *original_graph, preprocess_t preprocess_style) {
	GRAPH_TYPE *graph = (GRAPH_TYPE *) malloc(sizeof(GRAPH_TYPE));
	assert_malloc(graph);
	...
	for (UINT_t v=0; v<graph->numVertices; v++) {
		...
		for (UINT_t i=original_graph->rowPtr[v]; i<original_graph->rowPtr[v+1]; i++) {
...
int main(int argc, char **argv) {
	...
	GRAPH_TYPE *graph = preprocess_base(original_graph, preprocess_style);
